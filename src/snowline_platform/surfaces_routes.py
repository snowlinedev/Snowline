...
The surface set + allowlists come from `app.state.surface_listing` — frozen at
create_app when the gateway mounts were built, so the view can never drift
from what the gateway actually serves (and a post-boot env mutation can't
change or break this route). Composition uses the same `discover_upstreams`
the gateway aggregates with. Rides behind the trust middleware like every
platform route; the dashboard's Surfaces page is the consumer.
"""
...
async def list_surfaces(request: Request) -> dict:
    ...
    surfaces = []
    for name, allow in request.app.state.surface_listing:
