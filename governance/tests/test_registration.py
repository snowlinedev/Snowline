...
def test_heartbeat_confirms_first_beat_exactly_once(caplog):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json={"name": "governance"})

    with caplog.at_level(logging.INFO, logger="snowline_governance.registration"):
...
def test_httpx_filter_drops_only_heartbeat_post_plugins_lines():
    ...
    # letting every other httpx request trace through (governance's scope
    # reads + webhook delivery).
    from snowline_governance.app import _HeartbeatHttpxLogFilter
